...
const int8_t SEC_MARGIN_TOP_R = (ROTATE == 0)   ? SEC_MARGIN_TOP
                                ...
                                : (ROTATE == 2) ? SEC_MARGIN_BOTTOM
                                                : SEC_MARGIN_LEFT;
const int8_t SEC_MARGIN_RIGHT_R = (ROTATE == 0)   ? SEC_MARGIN_RIGHT
                                  : (ROTATE == 1) ? SEC_MARGIN_BOTTOM
                                  : (ROTATE == 2) ? SEC_MARGIN_LEFT
                                                  : SEC_MARGIN_TOP;
const int8_t SEC_MARGIN_BOTTOM_R = (ROTATE == 0)   ? SEC_MARGIN_BOTTOM
                                   : (ROTATE == 1) ? SEC_MARGIN_LEFT
                                   : (ROTATE == 2) ? SEC_MARGIN_TOP
                                                   : SEC_MARGIN_RIGHT;
const int8_t SEC_MARGIN_LEFT_R = (ROTATE == 0)   ? SEC_MARGIN_LEFT
...
#define EDGE_OF_ANGLE(angle)                                          \
  (((0 <= (angle) && (angle) <= M_PI / 4)                             \
    || (7 * (M_PI / 4) < (angle) && (angle) <= 2 * M_PI))             \
     ? TOP                                                            \
   : (M_PI / 4 < (angle) && (angle) <= 3 * (M_PI / 4))       ? RIGHT  \
   : (3 * (M_PI / 4) < (angle) && (angle) <= 5 * (M_PI / 4)) ? BOTTOM \
                                                             : LEFT)
...
#define HAND(cs, dc, mosi, sclk, rst, mot1, mot2, mot3, mot4, zero, zoff, max) \
  {                                                                            \
    .screen =                                                                  \
      Adafruit_SSD1351(SCREEN_WIDTH, SCREEN_HEIGHT, cs, dc, mosi, sclk, rst),  \
    .motor = AccelStepper(AccelStepper::FULL4WIRE, mot1, mot2, mot3, mot4),    \
    .max_time = max, .zero_pin = zero, .zero_off = zoff, .bitmap = {0},        \
    .time = -1, .err = false,                                                  \
  }
